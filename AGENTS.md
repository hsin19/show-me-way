...
- Components in `src/lib/components/`: `Timeline.svelte` (day event list), `TripOverview.svelte` (day-0 trip hub: phase-aware helper card — pre-trip prep progress / tonight's hotel / post-trip spend summary — plus the day list, hotels, tool entries, trip-profile switcher and share/settings entries), `DaySwitcher.svelte` (day navigation), `ToolsTab.svelte` (the 工具 tab: sticky sub-tab chips 準備/記帳/常用語/自訂行程 rendering snippet pages; without a loaded trip only 自訂行程 stays available), `Checklist.svelte` (packing/todo lists, on the 準備 page), `Ledger.svelte` (expense tracking with exchange rate, on the 記帳 page), `PhraseDeck.svelte` (survival-phrase deck, on the 常用語 page), `SettingsPanel.svelte` (the 自訂行程 page: YAML editor + backups + export — a page, NOT a modal; unsaved edits survive tab switches via `src/lib/settings-draft.svelte.ts`, and save/restore/reset navigate back to 行程), `HotelCards.svelte` (hotel/driver cards with the 當前入住 highlight, rendered on the overview), `WeatherBadge.svelte` (daily weather badge), `ChatPanel.svelte` (AI itinerary chat tab; prompts for the Gemini key on first use and keeps the conversation in memory only); brand map icons live in `src/lib/components/icons/`. The bottom nav has exactly three tabs — 行程/工具/AI; the overview's tool entries and phase card deep-link into 工具 sub-pages. Tools never open as sheets/modals — the only remaining overlays are `EnlargedCardOverlay`, `UpdatePrompt`, and `Toast`.
